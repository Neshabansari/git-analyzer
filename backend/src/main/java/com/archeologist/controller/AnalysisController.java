...
public class AnalysisController {
    ...
    @Autowired
    private com.archeologist.service.BugRiskService bugRiskService;
...
//            session.setAttribute("analysisId", saved.getId());
    ...
    public ResponseEntity<Map<String, Object>> processCommits(@RequestBody Map<String, Object> request) {
        ...
        .map(commit -> {
            ...
            // Add files (IMPORTANT for BugRisk)
            processedCommit.put("files", commit.get("files"));
...
//    }
    ...
    @GetMapping("/bug-risk")
public ResponseEntity<Map<String, Object>> getBugRisk(@RequestParam Long analysisId) {
    ...
    try {
        ...
        Map<String, Object> commitsJson = parseJson(analysis.getCommits());

        List<Map<String, Object>> commits = new ArrayList<>();

        if (commitsJson.containsKey("commits")) {
            commits = (List<Map<String, Object>>) commitsJson.get("commits");
        }

        List<Map<String, Object>> riskData =
                bugRiskService.calculateBugRisk(commits);
        ...
        logger.error("Error calculating bug risk", e);
